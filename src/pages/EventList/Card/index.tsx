...
export const Card = ({ event, reloadTrigger, toggleModal }: CardProps) => {
  ...
  const isImageURLValid = () => {
    if (event.urlImage?.endsWith(".png") || event.urlImage?.endsWith(".jpg")) {
      return true;
    }
    ...
    <div className="bg-white-w2 rounded-3xl shadow-md">
      <div className="flex">
        ...
        <div className="w-4/6 p-4 pr-6">
          <h2 className="font-gilroy-bold text-3xl text-black-b1 text-opacity-70">
            {event.title}
          </h2>
          <p className="font-gilroy-medium text-gray-g5 text-sm leading-4">
            {event.description}
          </p>
              ...
              <div className="flex items-center mb-3">
                <label className="switch">
                  <input
                    ...
                    onChange={(e) => updateEvent(e.currentTarget.checked)}
                  />
